...
local function setting_key_move()
    ...
    keymap(
        ...
        "N",
        "Nzz",
        opts("search previous match word with center screen")
    )
    ...
    keymap(
        ...
        "<C-o>",
        "<C-o>zz",
        opts("jump previous tag with center screen")
    )
    keymap(
        "n",
        "<C-i>",
        "<C-i>zz",
        opts("jump previous tag file with center screen")
    )
...
local function setting_key_buffer()
    ...
    keymap(
        ...
        "<leader><BS>",
        [[<cmd>%bd|e#|bd#|'<CR>|<CR>|`"]],
        opts("delete all other buffers")
    )
    keymap("n", "<C-c>", ":bd<CR>", opts("delete buffer"))
    ...
    keymap(
        "n",
        "<leader>c",
        ':lua require("config.function").ToggleQuickFix() <CR>',
        opts("open quickfix")
    )
    keymap(
        "n",
    ...
    keymap(
        "n",
        "<leader>w",
        [[<cmd> lua require("config.function").GetBuffers("list") <CR> | :b ]],
        opts("read buffer list")
    )
    keymap(
        "n",
        "<leader>q",
        [[<cmd> lua require("config.function").GetMarks("default") <CR>]],
        opts("read marks list")
    )
    keymap(
        "n",
        "<leader>h",
        [[<cmd> lua require("config.function").GetJumplist("default") <CR>]],
        opts("read jump list")
    )
    keymap(
        "n",
        "<leader>r",
        [[<cmd> lua require("config.function").GetRegisterList("default") <CR>]],
        opts("read register list")
    )
    keymap(
        "n",
        "<leader>te",
        [[<cmd> lua require("config.function").SetFileFormat() <CR>]],
...
local function setting_key_edit()
    ...
    keymap(
        "t",
        "cxz",
        "<C-\\><C-n><cmd>bp<CR><cmd>only<CR>",
        opts("switch to previous buffer")
    )
...
local function setting_key_yank()
    ...
    keymap(
        ...
        "X",
        '"_X',
        opts("delete backward character without save into register")
    )
    keymap(
        "v",
        "x",
        '"_x',
        opts("delete characters without save into register")
    )
    keymap(
        "v",
        "X",
        '"_X',
        opts("delete backward characters without save into register")
    )
    keymap("v", "p", '"_dp', opts(""))
end
    ...
    vim.keymap.set("n", "<Leader>E", function()
        if vim.fn.exists(":Explore") > 0 then
            ...
            keymap(
                ...
                "<Leader>E",
                ":e %:h/",
                opts_output("start from file directory")
            )
    ...
    vim.keymap.set("n", "<Leader>e", function()
        if vim.fn.exists(":Lexplore") > 0 then
            ...
            keymap(
                ...
                "<Leader>e",
                ":e ./",
                opts_output("start from current working directory")
            )
            vim.api.nvim_feedkeys(" e", "m", false)
        end
    end, opts(""))
...
local function setting_key_search()
    keymap(
        "n",
        "<Leader>fs",
        [[<cmd> lua require("features.string").SetupSearchOptions() <CR>]],
        opts("search options setup")
    )
    keymap("n", "<Leader>ff", [[:find ./**/*]], opts_output("find file"))
    keymap(
        "n",
        "<Leader>fF",
        [[<cmd> lua require("features.files").Search() <CR>]],
        opts("find files without ignore")
    )
    keymap(
        "n",
        "<Leader>fA",
        [[<cmd> lua require("features.string").Search(nil) <CR>]],
        opts("file all string")
    )
    keymap(
        "n",
        "<Leader>fa",
        [[<cmd> lua require("features.string").Search(nil, {case_sensitive = true}) <CR>]],
    ...
    keymap(
        "n",
        "<Leader>fW",
        [[<cmd> lua require("features.string").Search('') <CR>]],
        opts("find cursor word with all files")
    )
    keymap(
        "n",
        "<Leader>fc",
        [[<cmd> lua require("features.string").Search(nil, {extension = "*.c"}) <CR>]],
    ...
    keymap(
        "n",
        "<Leader>fh",
        [[<cmd> lua require("features.string").Search(nil, {extension = "*.h"}) <CR>]],
        opts("find word in .h")
    )
    keymap(
        "n",
        "<Leader>fD",
        [[<cmd> lua require("features.string").Search(nil, {extension = "{*.dts,*.dtsi}"}) <CR>]],
        opts("find word in .dts/.dtsi")
    )
    keymap(
        "n",
        "<Leader>fK",
        [[<cmd> lua require("features.string").Search(nil, {extension = "Kconfig"}) <CR>]],
        opts("find word in Kconfig")
    )
    keymap(
        "n",
        "<Leader>fk",
        [[<cmd> lua require("features.string").Search(nil, {extension = "*.conf"}) <CR>]],
        opts("find word in .conf")
    )
    keymap(
        "n",
        "<Leader>fm",
        [[<cmd> lua require("features.string").Search(nil, {extension = "CMakeLists.txt"}) <CR>]],
    ...
    keymap(
        "n",
        "<Leader>fv",
        [[<cmd> lua require("features.string").Search(nil, {extension=''}) <CR>]],
    ...
    keymap(
        "n",
        "<Leader>fb",
        [[<cmd> lua require("features.string").SearchByBuffer() <CR>]],
        opts("find word in current buffer")
    )
    keymap(
        "n",
        "<Leader>fd",
        (vim.fn.executable("rg") == 0)
                and [[<cmd> lua require("features.string").Search(nil, { extension = string.format("%s/../**/*", vim.fn.expand('%:h')), vimgrep = true }) <CR>]]
            or [[<cmd> lua require("features.string").Search(nil, {extension = string.format("%s/../", vim.fn.expand('%:h')), extension_is_regexp_path = true }) <CR>]],
    ...
    if vim.version().major < 1 or vim.version().minor < 10 then
        keymap(
            "n",
            "gcc",
            [[<cmd> lua require("features.string").ToggleComment() <CR>]],
            opts("comment a line")
        )
        keymap(
            "v",
            "gcc",
            [[<ESC> | gv | <cmd> lua require("features.string").ToggleComment() <CR> | <ESC>]],
...
local function setting_key_git()
    keymap(
        "n",
        "<Leader>ggl",
        [[<cmd> lua require("features.git").Log().Graph() <CR>]],
        opts("git log")
    )
    keymap(
        "n",
        "<Leader>ggL",
        [[<cmd> lua require("features.git").Log().Default() <CR>]],
        opts("git log with contents")
    )
    keymap(
        "n",
        "<Leader>ggH",
        [[<cmd> lua require("features.git").Log().Diff() <CR>]],
        opts("")
    )
    keymap(
        "n",
        "<Leader>ggC",
        [[<cmd> lua require("features.git").Log().CommitCount() <CR>]],
        opts("git commit count")
    )
    keymap(
        "n",
        "<Leader>ggd",
        [[<cmd> lua require("features.git").Diff().Default()<CR>]],
        opts("git diff on current file")
    )
    keymap(
        "n",
        "<Leader>ggD",
        [[<cmd> lua require("features.git").Diff().Latest() <CR>]],
        opts("git diff with commit")
    )
    keymap(
        "n",
        "<Leader>ggh",
        [[<cmd> lua require("features.git").Diff().Staged() <CR>]],
        opts("git diff with staged files")
    )
    keymap(
        "n",
        "<Leader>ggs",
        [[<cmd> lua require("features.git").Status().Default() <CR>]],
        opts("git status")
    )
    keymap(
        "n",
        "<Leader>ggS",
        [[<cmd> lua require("features.git").Status().Short() <CR>]],
        opts("git status with short information")
    )
    keymap(
        "n",
        "<Leader>ggw",
        [[<cmd> lua require("features.git").Status().ChkWhitespace() <CR>]],
        opts("git check whitespace")
    )
    keymap(
        "n",
        "<Leader>ggp",
        [[<cmd> lua require("features.git").Add().Patch() <CR>]],
        opts("git add")
    )
    keymap(
        "n",
        "<Leader>gga",
        [[<cmd> lua require("features.git").Add().Default() <CR>]],
        opts("git add with interactive")
    )
    keymap(
        "n",
        "<Leader>ggA",
        [[<cmd> lua require("features.git").Add().All() <CR>]],
        opts("git add all")
    )
    keymap(
        "n",
        "<Leader>ggc",
        [[<cmd> lua require("features.git").Commit().Default() <CR>]],
        opts("git commit")
    )
end
...
local function setting_key_terminal()
    keymap(
        "n",
        "<leader>ts",
        [[<cmd> lua require("config.function").Terminal("split") <CR>]],
        opts("open terminal with split window")
    )
    keymap(
        "n",
        "<leader>tv",
        [[<cmd> lua require("config.function").Terminal("vertical") <CR>]],
        opts("open terminal with vertical window")
    )
    keymap(
        "n",
        "<leader>tf",
        [[<cmd> lua require("config.function").Terminal("default") <CR>]],
        opts("open terminal")
    )
end
...
local function setting_key_session()
    ...
    keymap(
        "n",
        "<leader>os",
        [[<cmd> lua require("features.session").Get() <CR>]],
        opts("open session from opened editor path")
    )
    keymap(
        "n",
        "<leader>oS",
        [[<cmd> lua require("features.session").Select() <CR>]],
...
local function setting_key_features()
    keymap(
        "n",
        "<leader>tc",
        [[<cmd> lua require("config.function").CreateCtags() <CR>]],
        opts("create ctags")
    )
    keymap(
        "n",
        "<leader>tt",
        [[<cmd> lua require("config.function").ListFunctions() <CR>]],
        opts("lists current buffer functions")
    )
    keymap(
        "n",
        "<leader>b",
        [[<cmd> lua require("config.function").Build() <CR>]],
        opts("build")
    )
    keymap(
        "n",
        "<leader>B",
        [[<cmd> lua require("config.function").Build("latest") <CR>]],
        opts("build all")
    )
    keymap(
        "n",
        "<leader>vb",
        [[<cmd> lua require("features.bookmarks").Review() <CR>]],
        opts("review bookmarks")
    )
    keymap(
        "n",
        "<leader>sb",
        [[<cmd> lua require("features.bookmarks").Save() <CR>]],
        opts("save bookmark")
    )
    keymap(
        "n",
        "<leader>sB",
        [[<cmd> lua require("features.bookmarks").Rename() <CR>]],
        opts("rename bookmark")
    )
    keymap(
        "n",
        "<leader>Sb",
        [[<cmd> lua require("features.bookmarks").Remove() <CR>]],
        opts("remove bookmark")
    )
    keymap(
        "n",
        "<leader>SB",
        [[<cmd> lua require("features.bookmarks").Clear() <CR>]],
        opts("clear bookmark")
    )
    keymap(
        "n",
        "<leader>ob",
        [[<cmd> lua require("features.bookmarks").Get() <CR>]],
        opts("open current working directory bookmarks")
    )
    keymap(
        "n",
        "<leader>oB",
        [[<cmd> lua require("features.bookmarks").GetAll() <CR>]],
        opts("open all bookmarks")
    )
    keymap(
        "n",
        "<M-n>",
        [[<cmd> lua require("features.bookmarks").Next() <CR>zz]],
        opts("jump forward to bookmark")
    )
    keymap(
        "n",
        "<M-p>",
        [[<cmd> lua require("features.bookmarks").Prev() <CR>zz]],
    ...
    keymap(
        "n",
        "<leader>ot",
        [[<cmd> lua require("features.todo").Get() <CR>]],
        opts("open todo list")
    )
    keymap(
        "n",
        "<leader>st",
        [[<cmd> lua require("features.todo").Add() <CR>]],
    ...
    keymap(
        "n",
        "<leader>sm",
        [[<cmd> lua require('features.marks').Buffer('save') <CR>]],
        opts("create mark")
    )
    keymap(
        "n",
        "<leader>Sm",
        [[<cmd> lua require('features.marks').Buffer('remove') <CR>]],
        opts("remove mark")
    )
    keymap(
        "n",
        "<leader>Su",
        [[<cmd> lua require('features.marks').Buffer('clear') <CR>]],
        opts("clear marks")
    )
    keymap(
        "n",
        "<leader>om",
        [[<cmd> lua require('features.marks').Buffer() <CR>]],
        opts("open marks")
    )
    keymap(
        "n",
        "<leader>ou",
        [[<cmd> lua require('features.marks').Buffer('sort') <CR>]],
        opts("sort marks")
    )
    keymap(
        "n",
        "<leader>sM",
        [[<cmd> lua require('features.marks').All('save') <CR>]],
        opts("save marks")
    )
    keymap(
        "n",
        "<leader>SM",
        [[<cmd> lua require('features.marks').All('remove') <CR>]],
        opts("remove all marks")
    )
    keymap(
        "n",
        "<leader>SU",
        [[<cmd> lua require('features.marks').All('clear') <CR>]],
        opts("clear all marks")
    )
    keymap(
        "n",
        "<leader>oM",
        [[<cmd> lua require('features.marks').All() <CR>]],
        opts("open marks list")
    )
    keymap(
        "n",
        "<leader>oU",
        [[<cmd> lua require('features.marks').All('universal') <CR>]],
...
local function setting_key_working_directory()
    ...
    keymap(
        "n",
        "<leader>tww",
        [[<cmd> lua require('features.session').SaveWD() <CR>]],
        { silent = true, desc = "save current working directory" }
    )
    keymap(
        "n",
        "<leader>tws",
        [[<cmd> lua require('features.session').ChgSaveWD() <CR>]],
        {
            silent = true,
    ...
    keymap(
        "n",
        "<leader>twS",
        [[<cmd> lua require('features.session').SelWD() <CR>]],
        { silent = true, desc = "selection of saved working directory" }
    )
    keymap(
        "n",
        "<leader>twC",
        [[<cmd> lua require('features.session').ClrWD() <CR>]],
        { silent = true, desc = "clear all the saved working directory" }
    )
    keymap(
        "n",
        "<leader>twd",
        [[<cmd> lua require('features.session').DelWD() <CR>]],
