local function all_marks(act)
    ...
    for mark_to_insert = string.byte("A"), string.byte("Z"), 1 do
        ...
        if act == "save" then
            if
                (mark_empty == false) and (string.char(mark_to_insert) ~= "Z")
            then
                goto continue
            end
        ...
        elseif act == "remove" then
            if (mark_empty == true) or (string.char(mark_to_insert) == "Z") then
                ...
                if
                    (string.char(mark_to_insert) == "A")
                    or (
                        string.char(mark_to_insert) == "Z"
                        and mark_empty == false
                    )
                then
